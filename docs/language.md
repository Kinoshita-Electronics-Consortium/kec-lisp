...
| `(tls-connect host port [timeout-ms [insecure]])` | As `tcp-connect`, then a TLS handshake (OpenSSL). By default the peer certificate is verified: chain to a trusted root, plus a host-name or IP match, with OpenSSL's CN fallback disabled. A failure raises and names the reason. A truthy 4th argument (conventionally `':insecure`) skips verification for that connection, leaving it encrypted but unauthenticated. Returns the same kind of handle `tcp-connect` does, so the send/recv/close primitives are unchanged. `SSL_CERT_FILE` / `SSL_CERT_DIR` override the trust store. `FULL` only. |
...
| `(tcp-listen port [backlog])` | Bind and listen on **127.0.0.1** only, returning a listener handle (`SO_REUSEADDR` is set; `backlog` defaults to 8). Port 0 takes an ephemeral port, which `tcp-port` reads back. Present so tests can run hermetically. `FULL` only. |
...
| `(tcp-port handle)` | The local port this socket is bound to, via `getsockname`. Works on a listener or a connected socket, and is how `(tcp-listen 0)` reports the ephemeral port the kernel picked. `FULL` only. |
