...
A **host-agnostic editor/REPL extended-library tier** — the structural s-expr
tree buffer, cursor, keymap-as-data + dispatch, the token ranker, the REPL loop,
and serialize/load that host **knEmacs** and a strong standalone REPL — is being
established above Core (`provide`-gated, opt-in). It passes the same "runs on a
...
[ADR-0002](adr/ADR-0002-editor-repl-extended-library-tier.md).
